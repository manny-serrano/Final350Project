...
module camera_sccb_init #(
    ...
    // Helper to load next byte onto SDA during phase0 (SCL low)
    wire cur_bit = (state == ST_DEV)  ? (8'h42 << bit_idx)[7] :
                   (state == ST_REG)  ? cur_word[15 - bit_idx] :
                   (state == ST_DATA) ? cur_word[7  - bit_idx] :
                   1'b1;
